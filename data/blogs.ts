...
import { Blog, BlogPreview, NewBlog, UpdateBlog } from "@/lib/types";
...
export const getBlogByID = async (id: string): Promise<BlogPreview> => {
  const q = `
  SELECT blogs.*, users.username, users.img AS userimg FROM blogs 
  INNER JOIN users ON blogs.userid = users.id WHERE blogs.id = $1
  `;
